...
if [ -n "$TRANSCRIPT_PATH" ] && [ -f "$TRANSCRIPT_PATH" ]; then
    ...
    # 通知結果の貼り付け（【Claude Code）を除外
    REQUEST_TEXT=$(grep '"type":"user"' "$TRANSCRIPT_PATH" 2>/dev/null | jq -r 'select(.message.content | type == "string") | .message.content' | grep -v '【Claude Code' | tail -1 | head -c 200)
